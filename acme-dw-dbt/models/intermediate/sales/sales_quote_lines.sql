...
select
    ...
    h.manufacturer_code,
    l._ingested_at
from {{ ref('quote_lines_qut1') }}            l
left join {{ ref('sales_prices_quote') }}           p  on  p.doc_entry    = l.doc_entry
                                                      and p.line_number = l.line_number
left join {{ ref('sales_fiscal_usage_quote') }}       op on  op.doc_entry   = l.doc_entry
