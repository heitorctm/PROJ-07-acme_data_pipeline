...
-- effective_source_doc_entry (ActBaseEnt) is return-specific: points to the original invoice
select
    ...
    h.manufacturer_code,
    l._ingested_at
from {{ ref('sales_return_lines_rin1') }}            l
left join {{ ref('sales_prices_return') }}           p  on  p.doc_entry    = l.doc_entry
                                                      and p.line_number = l.line_number
left join {{ ref('sales_fiscal_usage_return') }}       op on  op.doc_entry   = l.doc_entry
