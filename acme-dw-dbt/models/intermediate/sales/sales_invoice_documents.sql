...
--
-- triangulation is exposed as an attribute but is NOT used as a filter here —
-- the legacy view vw_ranking_vendas_legada also does not filter by triangulation.
-- 85% of SAP invoices are set to 'C' (default state, in practice it does not mean
-- a pending classification). Filtering by 'N' would drop 99% of the real
-- revenue.
with invoices_dedup as (
    select *,
        row_number() over (partition by doc_entry order by _ingested_at desc) as rn
    from {{ ref('sales_invoice_oinv') }}
),
invoices as (
    select
        doc_entry,
        ...
        -- load_kit: 'P' (pending) is the SAP default state when the salesperson
        -- did not fill in the classification. we treat it as 'unclassified' (not
        -- as NULL nor as wholesale) — we make the missing data explicit so the
        -- sales team sees the leak and fixes it at the source.
        case load_kit
            when 'N' then 'retail'
        ...
        case triangulation
            when 'C' then 'to confirm'
            when 'S' then 'yes'
            when 'N' then 'no'
            else lower(triangulation)
        end                             as triangulation,
        _ingested_at
    from invoices_dedup
    where rn = 1
),
...
select
    ...
    -- cascade replicates V3: document store -> salesperson store
...
left join {{ ref('sales_freight_document') }}  fr on fr.document_type     = 'invoice'
